...
export default function SatuanPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/satuan?page=${page}&search=${encodeURIComponent(search)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/satuan/${editingId}`, formData);
      } else {
        await api.post('/satuan', formData);
      }
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data satuan ini?')) {
      try {
        await api.delete(`/satuan/${id}`);
        fetchData(currentPage);
      } catch (error) {
        console.error('Gagal menghapus data', error);
        toast.error('Gagal menghapus data. Data mungkin sedang digunakan oleh barang lain.');
      }
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Data Satuan</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola daftar satuan barang yang terdaftar di sistem.</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Kode Satuan</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data satuan.'}
                  </td>
                </tr>
              ) : (
                data.map((item: any, index: number) => (
                  <tr key={item.id_satuan || index} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-gray-500">{(currentPage - 1) * 10 + index + 1}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{item.kode_satuan || '-'}</td>
                    <td className="px-6 py-4 text-gray-900 font-medium">{item.nama_satuan || '-'}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      { label: 'Hapus', icon: Trash2, onClick: () => handleDelete(item.id_satuan), variant: 'danger' },
                    ]} />
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm font-sans">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Kode Satuan</label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="Misal: pcs, box, unit"
                />
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Satuan</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 focus:border-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900"
                  placeholder="Misal: Pieces, Kotak, Unit"
                />
              </div>
              <div>
