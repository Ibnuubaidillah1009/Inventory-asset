...
export default function MasterKategoriPage() {
  ...
  // Form State menyesuaikan gambar Anda
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/kategori?page=${page}&search=${encodeURIComponent(search)}`);
  ...
  const openModal = (item: any = null) => {
    if (item) {
      ...
      setEditingId(null);
      setFormData({ id_kategori: '', nama_kategori: '', keterangan: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus kategori ini?')) {
      try {
        ...
        toast.error('Gagal menghapus data. Kategori mungkin sedang digunakan.');
      }
    }
  };

  // FilteredData dihapus karena pencarian dilakukan di server-side
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Master Kategori</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola daftar kategori barang inventaris.</p>
        </div>
        <button
          onClick={() => openModal()}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data kategori.'}
                  </td>
                </tr>
              ) : (
                data.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-gray-900">{item.nama_kategori}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      <button onClick={() => handleDelete(item.id_kategori || item.id)} className="text-gray-400 hover:text-red-600 transition-colors cursor-pointer" title="Hapus">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">Detail Kategori</h3>
            ...
            <div className="p-6 space-y-4 text-sm">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Kode Kategori</label>
                <div className="text-gray-900 font-medium bg-gray-50 p-2 rounded-md border border-gray-100">{selectedItem.id_kategori || '-'}</div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Nama Kategori</label>
                <div className="text-gray-900 font-medium bg-gray-50 p-2 rounded-md border border-gray-100">{selectedItem.nama_kategori || '-'}</div>
              </div>
              <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Kode Kategori</label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="Misal: KTG-001"
                />
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Kategori</label>
                <input
                  type="text"
                  required
