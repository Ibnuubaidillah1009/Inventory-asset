...
export default function MerekPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/merek?page=${page}&search=${encodeURIComponent(search)}`);
      ...
      console.error('Gagal mengambil data merek', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/merek/${editingId}`, formData);
      } else {
        await api.post('/merek', formData);
      }
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data merek ini?')) {
      try {
        await api.delete(`/merek/${id}`);
        fetchData(currentPage);
      } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Data Merek</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola daftar merek barang yang terdaftar di sistem.</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Nama Merek</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data merek.'}
                  </td>
                </tr>
              ) : (
                data.map((item: any, index: number) => (
                  <tr key={item.id_merek || index} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-gray-500">{(currentPage - 1) * 10 + index + 1}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{item.nama_merek || '-'}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      { label: 'Hapus', icon: Trash2, onClick: () => handleDelete(item.id_merek), variant: 'danger' },
                    ]} />
                    </td>
                  </tr>
                ))
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 shrink-0">
              <h3 className="text-lg font-semibold text-gray-900 font-sans">
                {editingId ? 'Edit Merek' : 'Tambah Merek'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm font-sans">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Merek</label>
                <input
                  type="text"
                  required
                  value={formData.nama_merek}
                  onChange={(e) => setFormData({ ...formData, nama_merek: e.target.value })}
                  ...
                  placeholder="Misal: Asus, Lenovo, Honda"
                />
              </div>
              <div>
