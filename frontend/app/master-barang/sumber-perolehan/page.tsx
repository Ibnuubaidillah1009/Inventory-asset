...
export default function SumberPerolehanPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/sumber-perolehan?page=${page}&search=${encodeURIComponent(search)}`);
      ...
      console.error('Gagal mengambil data sumber perolehan', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openModal = (item: any = null) => {
    if (item) {
      setEditingId(item.id_sumber_perolehan);
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/sumber-perolehan/${editingId}`, formData);
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data sumber perolehan ini?')) {
      try {
        await api.delete(`/sumber-perolehan/${id}`);
        fetchData(currentPage);
      } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Sumber Perolehan</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola daftar sumber pendanaan / perolehan aset sekolah.</p>
        </div>
        <button
          onClick={() => openModal()}
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            type="text"
            placeholder="Cari sumber perolehan..."
            value={searchQuery}
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Kode Sumber</th>
                <th className="px-6 py-4 font-medium">Nama Sumber</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data sumber perolehan.'}
                  </td>
                </tr>
              ) : (
                data.map((item: any, index: number) => (
                  <tr key={item.id_sumber_perolehan || index} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-gray-500">{(currentPage - 1) * 10 + index + 1}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{item.kode_sumber || '-'}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 shrink-0">
              <h3 className="text-lg font-semibold text-gray-900 font-sans">
                {editingId ? 'Edit Sumber Perolehan' : 'Tambah Sumber Perolehan'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm font-sans">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Kode Sumber</label>
                <input
                  type="text"
                  required
                  value={formData.kode_sumber}
                  onChange={(e) => setFormData({ ...formData, kode_sumber: e.target.value })}
                  ...
                  placeholder="Misal: BOS, APBD, HIBAH"
                />
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Sumber</label>
                <input
                  type="text"
                  required
                  value={formData.nama}
                  ...
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 focus:border-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900"
                  placeholder="Misal: Bantuan Operasional Sekolah"
                />
              </div>
              <div>
