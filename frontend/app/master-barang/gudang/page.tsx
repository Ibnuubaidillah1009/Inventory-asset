...
export default function GudangPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/gudang?page=${page}&search=${encodeURIComponent(search)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/gudang/${editingId}`, formData);
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data gudang ini?')) {
      try {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Data Gudang</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola data ruang gudang penyimpanan barang sekolah.</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Kode Gudang</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data gudang.'}
                  </td>
                </tr>
              ) : (
                data.map((item: any, index: number) => (
                  <tr key={item.kode_gudang || index} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-gray-500">{(currentPage - 1) * 10 + index + 1}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{item.kode_gudang || '-'}</td>
                    <td className="px-6 py-4 text-gray-900 font-medium">{item.nama_gudang || '-'}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      { label: 'Hapus', icon: Trash2, onClick: () => handleDelete(item.kode_gudang), variant: 'danger' },
                    ]} />
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm font-sans">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Kode Gudang</label>
                <input
                  type="text"
                  required
                  disabled={!!editingId}
                  ...
                  placeholder="Misal: GDG-A, GDG-UTAMA"
                />
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Gudang</label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="Misal: Gudang Alat Elektronik, Gudang ATK"
                />
              </div>
              <div>
