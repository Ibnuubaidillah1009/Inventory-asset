...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // State untuk mengontrol dropdown buka/tutup dinamis
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (treeString) {
          ...
          menuTree.forEach((parent: any) => {
            if (parent.nama_modul === 'AUTENTIKASI') return; // Skip auth module dari sidebar
            ...
            if (parent.children_recursive && parent.children_recursive.length > 0) {
              const subItems = parent.children_recursive.map((child: any) => {
                const path = child.nama_modul.toLowerCase().replace(/_/g, '-').replace(/ /g, '-');
              ...
              const path = parent.nama_modul.toLowerCase().replace(/_/g, '-').replace(/ /g, '-');
              dynamicMenus.push({
                label: parentLabel,
                href: `/dashboard/${path}`,
                icon: parentIcon,
        ...
        // Otomatis buka dropdown jika user sedang berada di dalam salah satu halaman Master Data
    ...
    loadUserAndAkses();
  }, [pathname, handleInvalidSession]); // Tambahkan pathname sebagai dependency agar dropdown responsif
        ...
        <div className="flex-1 overflow-y-auto py-4">
          <nav className="space-y-1 px-3">
            {allowedMenus.map((menu, index) => {
              ...
              // RENDER JIKA MENU ADALAH DROPDOWN (Punya subItems)
              if (menu.subItems) {
                return (
              ...
              // RENDER JIKA MENU BIASA (Tidak ada subItems)
