...
export default function AksesPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/akses?page=${page}&search=${encodeURIComponent(search)}`);
  ...
  const openModal = (item: any = null) => {
    if (item) {
      setEditingId(item.id_akses || item.id);
      setFormData({ nama_modul: item.nama_modul || '' });
    } else {
      setEditingId(null);
      setFormData({ nama_modul: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/akses/${editingId}`, formData);
      } else {
        await api.post('/akses', formData);
      }
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus akses ini?')) {
      try {
        ...
        toast.error('Gagal menghapus data. Akses mungkin sedang digunakan.');
      }
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Manajemen Akses</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola level akses modul sistem.</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">ID Akses</th>
                <th className="px-6 py-4 font-medium">Nama Modul</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data akses.'}
                  </td>
                </tr>
              ) : (
                data.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900">{item.id_akses || '-'}</td>
                    <td className="px-6 py-4 text-gray-900">{item.nama_modul}</td>
                    <td className="px-6 py-4 text-right">
                      ...
                      <button onClick={() => handleDelete(item.id_akses || item.id)} className="text-gray-400 hover:text-red-600 transition-colors cursor-pointer" title="Hapus">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">Detail Akses</h3>
            ...
            <div className="p-6 space-y-4 text-sm">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">ID Akses</label>
                <div className="text-gray-900 font-medium bg-gray-50 p-2 rounded-md border border-gray-100">{selectedItem.id_akses || '-'}</div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Nama Modul</label>
                <div className="text-gray-900 font-medium bg-gray-50 p-2 rounded-md border border-gray-100">{selectedItem.nama_modul || '-'}</div>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">
                {editingId ? 'Edit Akses' : 'Tambah Akses'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Modul</label>
                <input
                  type="text"
                  required
