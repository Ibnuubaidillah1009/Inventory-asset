...
const statusBadge: Record<string, string> = {
  dilaporkan: 'bg-red-100 text-red-800',
...
export default function KerusakanPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const res = await api.get(`/kerusakan?page=${page}&search=${encodeURIComponent(search)}`).catch(() => ({ data: { data: [] } }));
  ...
  const openModal = (item: any = null) => {
    if (item) {
      ...
      setEditingId(null);
      setFormData({ kode_barang: '', tanggal_lapor: new Date().toISOString().split('T')[0], kondisi: '', keterangan: '', status: 'dilaporkan' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/kerusakan/${editingId}`, formData);
      } else {
        await api.post('/kerusakan', formData);
      }
      toast.success('Data berhasil disimpan');
      closeModal();
      fetchData();
    } catch (error: any) {
      toast.error(error?.response?.data?.message || 'Gagal menyimpan data kerusakan.');
    } finally { setIsSubmitting(false); }
  };

  const handleDelete = async (id: number) => {
    if (!window.confirm('Apakah Anda yakin ingin menghapus laporan kerusakan ini?')) return;
    try {
      await api.delete(`/kerusakan/${id}`);
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Kerusakan Aset</h1>
          <p className="text-sm text-gray-500 mt-1">Catat dan kelola laporan kerusakan barang inventaris.</p>
        </div>
        <button onClick={() => openModal()} className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors bg-gray-900 text-white hover:bg-gray-800 h-10 px-4 py-2 cursor-pointer">
          <Plus className="mr-2 h-4 w-4" /> Tambah Kerusakan
        </button>
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Tanggal Lapor</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan={8} className="px-6 py-12 text-center text-gray-500">{searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data kerusakan.'}</td></tr>
              ) : data.map((item, i) => (
                <tr key={item.id || i} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-500">{item.tanggal_lapor || '-'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">Detail Kerusakan</h3>
            ...
            <div className="p-6 space-y-4 text-sm">
              {[
                ...
                ['Tanggal Lapor', selectedItem.tanggal_lapor],
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">{editingId ? 'Edit Kerusakan' : 'Tambah Kerusakan'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
              ...
              <div>
                <label className="block font-medium text-gray-700 mb-1">Tanggal Lapor *</label>
                <input type="date" required value={formData.tanggal_lapor} onChange={e => setFormData({ ...formData, tanggal_lapor: e.target.value })} className={inputClass} />
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Kondisi *</label>
                <input type="text" required value={formData.kondisi} onChange={e => setFormData({ ...formData, kondisi: e.target.value })} className={inputClass} placeholder="Kondisi barang" />
              </div>
              <div>
