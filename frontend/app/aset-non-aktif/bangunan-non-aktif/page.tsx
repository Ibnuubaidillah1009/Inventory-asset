...
export default function BangunanNonAktifPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const res = await api.get(`/bangunan-non-aktif?page=${page}&search=${encodeURIComponent(search)}`).catch(() => ({ data: { data: [] } }));
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Bangunan Non-Aktif</h1>
        <p className="text-sm text-gray-500 mt-1">Daftar aset bangunan yang sudah tidak aktif.</p>
      </div>

      <div className="relative w-full sm:max-w-md">
        <Search className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
        <input type="text" placeholder="Cari kode atau nama bangunan..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md bg-white placeholder-gray-500 focus:outline-none focus:border-gray-900 focus:ring-1 focus:ring-gray-900 sm:text-sm" />
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200 uppercase">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">Kode Bangunan</th>
                <th className="px-4 py-3 font-medium">Nama Bangunan</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-500">Tidak ada data bangunan non-aktif.</td></tr>
              ) : data.map((item, i) => (
                <tr key={item.id || i} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-gray-700">{item.kode_bangunan || '-'}</td>
                  <td className="px-4 py-3 font-medium text-gray-900">{item.nama_bangunan || '-'}</td>
