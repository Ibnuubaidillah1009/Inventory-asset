...
export default function PenghapusanAsetPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/penghapusan-aset?page=${page}&search=${encodeURIComponent(search)}`);
      ...
      console.error('Gagal mengambil data penghapusan aset', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchAsetList = async () => {
    try {
      ...
      setAsetList(extractData(response.data.data));
    } catch (error) {
  ...
  const openModal = () => {
    setFormData({ kode_barang: '', tanggal_hapus: '', alasan_hapus: '' });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/penghapusan-aset', formData);
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data penghapusan ini?')) {
      try {
        await api.delete(`/penghapusan-aset/${id}`);
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Penghapusan Aset</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola penghapusan aset dari sistem.</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Tanggal Hapus</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data penghapusan aset.'}
                  </td>
                </tr>
              ) : (
                data.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900">{item.id_penghapusan || item.id}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">Tambah Penghapusan Aset</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
              <div>
                ...
                >
                  ...
                  {asetList.map((aset: any) => (
                    <option key={aset.id} value={aset.kode_barang || aset.kode}>
                      {aset.kode_barang || aset.kode} - {aset.nama_barang || aset.nama}
                    </option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Tanggal Hapus</label>
                <input
                  type="date"
                  required
                  value={formData.tanggal_hapus}
                  onChange={(e) => setFormData({ ...formData, tanggal_hapus: e.target.value })}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 focus:border-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900"
                />
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Alasan Penghapusan</label>
                <textarea
                  rows={3}
                  required
                  value={formData.alasan_hapus}
                  onChange={(e) => setFormData({ ...formData, alasan_hapus: e.target.value })}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 focus:border-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900"
