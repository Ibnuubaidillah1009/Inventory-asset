...
export default function NonAktifBarangPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [resNonAktif, resAset, resStatus] = await Promise.all([
  ...
  const openModal = (item: any = null) => {
    if (item) {
      setEditingId(item.id_barang_non_aktif);
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/barang-non-aktif/${editingId}`, payload);
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Hapus data barang non-aktif ini?')) {
      try { await api.delete(`/barang-non-aktif/${id}`); fetchData(); } catch { alert('Gagal menghapus.'); }
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight flex items-center gap-2"><Ban className="h-6 w-6" /> Non Aktif Barang</h1>
          <p className="text-sm text-gray-500 mt-1">Non-aktifkan barang dari inventaris aktif.</p>
        </div>
        <button onClick={() => openModal()} className="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors bg-gray-900 text-white hover:bg-gray-800 h-10 px-4 py-2 cursor-pointer">
          <Plus className="mr-2 h-4 w-4" /> Tambah Non Aktif
        </button>
      </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-500">Belum ada data barang non-aktif.</td></tr>
              ) : filtered.map((item, i) => (
                <tr key={item.id_barang_non_aktif || i} className="hover:bg-gray-50 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">{editingId ? 'Edit Non Aktif' : 'Tambah Non Aktif'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
              ...
              <div>
                ...
                <select required value={formData.id_status} onChange={(e) => setFormData({ ...formData, id_status: e.target.value })} className={inputClass}>
                  <option value="" disabled>Pilih Status</option>
                  {statusList.map((s) => <option key={s.id_status} value={s.id_status}>{s.nama_status || s.nama}</option>)}
                </select>
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Jumlah Non Aktif</label>
                <input type="number" min="1" value={formData.jumlah_nonaktif} onChange={(e) => setFormData({ ...formData, jumlah_nonaktif: e.target.value })} className={inputClass} />
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Tanggal *</label>
                <input type="date" required value={formData.tanggal} onChange={(e) => setFormData({ ...formData, tanggal: e.target.value })} className={inputClass} />
              </div>
              <div>
                <label className="block font-medium text-gray-700 mb-1">Keterangan</label>
                <textarea rows={3} value={formData.keterangan} onChange={(e) => setFormData({ ...formData, keterangan: e.target.value })} className={inputClass} placeholder="Alasan non-aktif..." />
              </div>
