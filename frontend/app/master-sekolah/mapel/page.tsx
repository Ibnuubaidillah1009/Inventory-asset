...
export default function MapelPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/mapel?page=${page}&search=${encodeURIComponent(search)}`);
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data mata pelajaran ini?')) {
      try {
        ...
        toast.error('Gagal menghapus data. Mata pelajaran mungkin masih digunakan oleh data pengguna (guru).');
      }
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Mata Pelajaran (Mapel)</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola data mata pelajaran yang diajarkan di sekolah.</p>
        </div>
        <button
          onClick={() => openModal()}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data mata pelajaran.'}
                  </td>
                </tr>
              ) : (
                data.map((item: any, index: number) => (
                  <tr key={item.id_mapel || index} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-gray-500">{(currentPage - 1) * 10 + index + 1}</td>
                    <td className="px-6 py-4 font-medium text-gray-900">{item.nama_mapel || '-'}</td>
                    <td className="px-6 py-4 text-right">
                                          <DropdownMenu actions={[
                      { label: 'Ubah', icon: Pencil, onClick: () => openModal(item), variant: 'default' },
                      { label: 'Hapus', icon: Trash2, onClick: () => handleDelete(item.id_mapel), variant: 'danger' },
                    ]} />
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm font-sans">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Mata Pelajaran</label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="Misal: Matematika, Pemrograman Web"
                />
              </div>
