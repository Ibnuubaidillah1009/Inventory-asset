...
export default function JurusanPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/jurusan?page=${page}&search=${encodeURIComponent(search)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/jurusan/${editingId}`, formData);
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data jurusan ini?')) {
      try {
        ...
        toast.error('Gagal menghapus data. Jurusan mungkin masih terikat pada rombel sekolah.');
      }
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Data Jurusan</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola program keahlian / jurusan yang ada di sekolah.</p>
        </div>
        <button
          onClick={() => openModal()}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data jurusan.'}
                  </td>
                </tr>
              ) : (
                data.map((item: any, index: number) => (
                  <tr key={item.id_jurusan || index} className="hover:bg-gray-50 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm font-sans">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Jurusan</label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="Misal: Rekayasa Perangkat Lunak, Akuntansi"
                />
              </div>
