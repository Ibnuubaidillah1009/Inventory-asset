...
export default function TahunAjaranPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const response = await api.get(`/tahun-ajaran?page=${page}&search=${encodeURIComponent(search)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/tahun-ajaran/${editingId}`, formData);
      } else {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Tahun Ajaran</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola daftar tahun ajaran.</p>
        </div>
        <button
          onClick={() => openModal()}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data tahun ajaran.'}
                  </td>
                </tr>
              ) : (
                data.map((item, index) => (
