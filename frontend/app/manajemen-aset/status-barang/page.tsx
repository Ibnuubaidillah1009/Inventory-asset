...
export default function StatusBarangPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    ...
    try {
      ...
      console.error('Gagal mengambil data status barang', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openModal = (item: any = null) => {
    if (item) {
      setEditingId(item.id_status || item.id);
      setFormData({
        nama_status: item.nama_status || '',
        keterangan: item.keterangan || '' });
    } else {
      setEditingId(null);
      setFormData({ nama_status: '', keterangan: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus status barang ini?')) {
      try {
        ...
        toast.error('Gagal menghapus data. Status barang mungkin sedang digunakan.');
      }
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Master Status Barang</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola daftar status barang untuk pelacakan inventaris.</p>
        </div>
        <button
          onClick={() => openModal()}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data status barang.'}
                  </td>
                </tr>
              ) : (
                data.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900">{item.id_status || '-'}</td>
                    <td className="px-6 py-4 text-gray-900">{item.nama_status}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      { label: 'Hapus', icon: Trash2, onClick: () => handleDelete(item.id_status || item.id), variant: 'danger' },
                    ]} />
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">Detail Status Barang</h3>
            ...
            <div className="p-6 space-y-4 text-sm">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">ID Status</label>
                <div className="text-gray-900 font-medium bg-gray-50 p-2 rounded-md border border-gray-100">{selectedItem.id_status || '-'}</div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Nama Status</label>
                <div className="text-gray-900 font-medium bg-gray-50 p-2 rounded-md border border-gray-100">{selectedItem.nama_status || '-'}</div>
              </div>
              <div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">
                {editingId ? 'Edit Status' : 'Tambah Status'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Status</label>
                <input
                  type="text"
                  required
