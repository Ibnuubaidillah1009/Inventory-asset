...
export default function RuangPage() {
  ...
  const fetchData = async (page = 1, search = searchQuery) => {
    setLoading(true);
    try {
      const [resRuang, resLokasi] = await Promise.all([
        api.get(`/ruang?page=${page}&search=${encodeURIComponent(search)}`),
  ...
  const openModal = (item: any = null) => {
    if (item) {
      setEditingId(item.id_ruang || item.id);
      setFormData({
        id_lokasi: item.id_lokasi || '',
        nama_ruang: item.nama_ruang || '' });
    } else {
      setEditingId(null);
      setFormData({ id_lokasi: '', nama_ruang: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/ruang/${editingId}`, formData);
      } else {
        await api.post('/ruang', formData);
      }
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus ruang ini?')) {
      try {
        ...
        toast.error('Gagal menghapus data. Ruang mungkin sedang digunakan.');
      }
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Master Ruang</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola daftar ruangan di lingkungan sekolah.</p>
        </div>
        <button
          onClick={() => openModal()}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                    {searchQuery ? 'Tidak ada data yang cocok dengan pencarian Anda.' : 'Belum ada data ruang.'}
                  </td>
                </tr>
              ) : (
                data.map((item, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900">{item.id_ruang || '-'}</td>
                    <td className="px-6 py-4 text-gray-900">{item.nama_ruang}</td>
                    <td className="px-6 py-4 text-gray-500">{item.lokasi?.nama_lokasi || '-'}</td>
                    <td className="px-6 py-4 text-right">
                      ...
                      { label: 'Hapus', icon: Trash2, onClick: () => handleDelete(item.id_ruang || item.id), variant: 'danger' },
                    ]} />
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">Detail Ruang</h3>
            ...
            <div className="p-6 space-y-4 text-sm">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">ID Ruang</label>
                <div className="text-gray-900 font-medium bg-gray-50 p-2 rounded-md border border-gray-100">{selectedItem.id_ruang || '-'}</div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Nama Ruang</label>
                <div className="text-gray-900 font-medium bg-gray-50 p-2 rounded-md border border-gray-100">{selectedItem.nama_ruang || '-'}</div>
              </div>
              <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-sm">
              <div>
                <label className="block font-medium text-gray-700 mb-1">Nama Ruang</label>
                <input
                  type="text"
                  required
              ...
              <div>
                ...
                >
                  ...
                  {lokasiList.map((lok) => (
                    <option key={lok.id_lokasi || lok.id} value={lok.id_lokasi || lok.id}>
                      {lok.nama_lokasi}
                    </option>
                  ))}
                </select>
